...
def make_dataset(input_ids, input_masks, input_segment_ids, answer_lables, clinical_feature_ids):
    ...
    all_clinical_feature_ids = torch.tensor([clinical_feature_id for clinical_feature_id in clinical_feature_ids], dtype=torch.long)       
    return TensorDataset(all_input_ids, all_input_masks, all_input_segment_ids, all_answer_lables, all_clinical_feature_ids)
...
def convert_data_to_feature(tokenizer, train_data_path):
    ...
    for qa_pair in qa_pairs:
        ...
        try:
            ...
            questions.append(q)
            answers.append(a)
            clinical_features.append(cf)
        except:
            continue
    
    assert len(answers) == len(questions) == len(clinical_features)
    
    ans_dic = DataDic(answers)
    question_dic = DataDic(questions)
    clinical_feature_dic = DataDic(clinical_features)
    ...
    cf_tokens = []
    max_seq_len = 0
    for cf in clinical_feature_dic.data:
        feature_ids = to_bert_ids(tokenizer,cf)
        if(len(feature_ids)>max_seq_len):
            max_seq_len = len(feature_ids)
        cf_tokens.append(feature_ids)
    assert max_seq_len <= 512
    
    for cf in cf_tokens:
        while len(cf)<max_seq_len:
            cf.append(0)
    ...
    input_ids = q_tokens
    clinical_feature_ids = cf_tokens
    input_masks = [[1]*max_seq_len for i in range(len(question_dic))]
    input_segment_ids = [[0]*max_seq_len for i in range(len(question_dic))]
    assert len(input_ids) == len(question_dic) and len(input_ids) == len(input_masks) and len(input_ids) == len(input_segment_ids) and len(clinical_feature_ids) == len(clinical_feature_dic) and len(clinical_feature_ids) == len(input_masks) and len(clinical_feature_ids) == len(input_segment_ids)
    ...
    data_features = {'input_ids':input_ids,
                    ...
                    'answer_dic':ans_dic,
                    'clinical_feature_dic':clinical_feature_dic}
